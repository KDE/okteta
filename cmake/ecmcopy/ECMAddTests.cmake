...
the current source directory, unless the ``DIR`` argument is passed to target
another directory. The ``<dir>`` value can also refer to a source directory
which itself does not have a CMakeLists.txt file and thus is not added by any
`add_subdirectory()
<https://cmake.org/cmake/help/latest/command/add_subdirectory.html>`_ call.
...
when deriving the name prefix. By default the varibale is set to
...
function(ecm_test_set_dir_properties)
    ...
    if(_subdir)
        file(RELATIVE_PATH _rel_path ${CMAKE_CURRENT_SOURCE_DIR} ${_subdir})
        string(SHA1 _rel_path_hash ${_rel_path})
        set(_property_name_suffix ":/${_rel_path_hash}")
    else()
        set(_property_name_suffix)
    endif()
