...
class StaticLengthArrayDataInformation: public DataInformationWithChildren
{
Q_OBJECT
protected:
...
public:
    ...
    StaticLengthArrayDataInformation(QString name, unsigned int length,
            const DataInformation& childItem, int index = -1, DataInformation* parent =
                    NULL);
...
public:
    ...
    {
        return hasChildren() ? childCount() * mChildren[0]->getSize() : 0;
    }
