  ...
  : KDialog( parent )
{
    ...
    const QString caption =
        i18nc("@title:window","Set Bytes per Line");
    setCaption( caption );
}

int BytesPerLineDialog::bytesPerLine()      const { return mBytesPerLineEdit->value(); }
int BytesPerLineDialog::groupedBytesCount() const { return mGroupedBytesCountEdit->value(); }
...
void BytesPerLineDialog::setGroupedBytesCount( int groupedBytesCount )
{
